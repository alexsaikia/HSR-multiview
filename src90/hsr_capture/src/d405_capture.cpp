...
public:
  ...
  {
    this->declare_parameter<std::string>("data_dir", "/home/kukasrv/data/dataset16/r300/");
...
int main(int argc, char *argv[])
{
  ...
  sprintf(pose_buffer, "%sposes/linkcamera_poses.csv", (node->get_parameter("data_dir")).as_string().c_str());
  ...
  sprintf(pose_buffer, "%sposes/linkFlangePosition.csv", (node->get_parameter("data_dir")).as_string().c_str());
  ...
  color_poses << "Image #,"
        ...
        << "Yaw,"
        << "\n";
  std::string color_frame = "iiwa_link_camera";
  ...
  depth_poses << "Image #,"
        ...
        << "Yaw,"
        << "\n";
  std::string depth_frame = "camera_color_optical_frame";
  ...
  const int N = 10;
  const double polar_range = 0.8 * PI / 2;
  ...
  for (int i = 0; i < N; i++)
  {
    for (int j = 0; j < N; j++)
    {
      ...
      try{
        ...
        RCLCPP_INFO(logger, "It's messed up at the pose target setting");
      }
      ...
      if (success)
      {
        ...
        // sleep(1);
          ...
          RCLCPP_ERROR(logger, "Capture Failure");
        }
